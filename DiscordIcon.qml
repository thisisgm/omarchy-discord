...
// The Discord mark as vector geometry, so it stays crisp at any bar size and
// takes the theme foreground like the first-party Dropbox and Tailscale icons.
Item {
  id: root
  ...
  // Measured ink bounds of the path below inside its 24x24 viewBox. Fitting to
  // these rather than the viewBox is what keeps the mark at the same optical
  // weight as the Nerd Font glyphs beside it.
  ...
  Shape {
    ...
    ShapePath {
      ...
      strokeWidth: 0
      // The eyes are subpaths inside the body; odd-even makes them holes
      // whichever way they are wound.
      fillRule: ShapePath.OddEvenFill

      // The mark's 17 corner arcs are radius <0.08 in a 24-unit box, so they
      // are drawn as lines: identical at bar sizes, and Qt's path parser
      // mis-reads the compact "00" arc-flag form the original used.
      PathSvg {
